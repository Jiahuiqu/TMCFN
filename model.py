...
from torch_geometric.nn import EdgeConv, DenseGCNConv, DenseGraphConv, GCNConv, GATConv, SAGEConv, GINConv
import copy
...
##
class Graph_ecoder(torch.nn.Module):
    def __init__(self, num_node_features, num_classes):
        super(Graph_ecoder, self).__init__()
...
##
class Attention_Feature_Fusion_Module_1_ViT(torch.nn.Module):
    def __init__(self):
        super(Attention_Feature_Fusion_Module_1_ViT, self).__init__()
    ...
    def forward(self, x1, x2):
        ...
        # out = self.MLP_3(torch.squeeze(x1) * alpha + torch.squeeze(x2) * (1 - alpha))
        add_x = torch.squeeze(x1) * alpha + torch.squeeze(x2) * (1 - alpha)
        out = self.MLP_3(torch.cat([x1, add_x, x2], dim=-1))
        return out


class Contrastive_Fusion_Feature_Learning_Module_ViT_1(torch.nn.Module):
    def __init__(self, img_size, input_channels, patch_size, dim, depth, heads, mlp_dim,
        ...
        super(Contrastive_Fusion_Feature_Learning_Module_ViT_1, self).__init__()
        self.ViT_Encoder_online = VisionTransformer(img_size, input_channels, patch_size, dim, depth, heads, mlp_dim,
                                             dropout, dim_head)  ##32 -- 64
        self.AFFM_online = Attention_Feature_Fusion_Module_1_ViT()
...
class Vision_Net_Final(nn.Module):
    def __init__(self, hparams, patchsize_HSI, device1, device2, class_num):
        ...
        self.cc = Contrastive_Fusion_Feature_Learning_Module_ViT_1(img_size=patchsize_HSI, input_channels=32,patch_size=2, dim=32, depth=4, heads=4, mlp_dim=32, dropout=0.3, dim_head=32).to(self.device1)

        self.graph_encoder1 = Graph_ecoder(32, 32).to(self.device2)

        ## 图分支
        self.Graph_DCE = DGM_Model(hparams).to(self.device2)  # graph dynamic construction and embedding
        self.vision_fusion = Text_Guided_Attention_Feature_Fusion_Module().to(self.device2)



    def forward(self,init_edge=None, patch_hsi=None, patch_lidar=None, ft=None, labeled_mask=None, warm_up=False, len_labeled=None):

        x1, x2, x3,loss_cl = self.cc(patch_hsi, patch_lidar+patch_hsi, patch_lidar)

        if(len_labeled == None):
            x1 = x1[labeled_mask]  ## torch.Size([ N , 32])
            x2 = x2[labeled_mask]
            x3 = x3[labeled_mask]
        else:
            x1 = x1[0:len_labeled, :][labeled_mask]  ## torch.Size([ N , 32])
        ...
        init_edge = utils.get_adj(x1.detach(), x2.detach(), x3.detach(), self.device, 20).to(self.device2)

        x1 = torch.unsqueeze(x1, dim=0)  ## torch.Size([1, N , 32])
        x2 = torch.unsqueeze(x2, dim=0)
        x3 = torch.unsqueeze(x3, dim=0)

        x1, x2, x3, edges, logprobs = self.Graph_DCE(x1, x2, x3, init_edge,warm_up=warm_up)

        x1 = torch.squeeze(x1, dim=0)  ## torch.Size([1, N , 32])
        x2 = torch.squeeze(x2, dim=0)
        x3 = torch.squeeze(x3, dim=0)

        Z_image = self.vision_fusion(x1, x2, x3, ft)

        return Z_image, loss_cl, logprobs
...
class Model_All(torch.nn.Module):
    def __init__(self, hparams,  patchsize_HSI, device1, device2, class_num):
        ...
        self.projector_vision = nn.Sequential(nn.Linear(32, 512)).to(self.device2)

    def forward(self, patch_hsi,patch_lidar, labeled_mask, text, warm_up, len_labeled=None):

        with torch.no_grad():
            text = clip.tokenize(text).to(self.device2)
            feature_text = self.text_net(text)

        feature_vision,loss_vision_CL,logprobs = self.vision_net(patch_hsi=torch.squeeze(patch_hsi).to(self.device1),
                                    patch_lidar=torch.squeeze(patch_lidar).to(self.device1), ft=feature_text,
                                    labeled_mask=labeled_mask, warm_up=warm_up, len_labeled=len_labeled)

        Z_vison = self.projector_vision(feature_vision)
        Z_text = feature_text
