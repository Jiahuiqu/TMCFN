...
def get_data(dataset_name='muufl'):
    if dataset_name == 'muufl':
        print('Muufl......')
        hsi = sio.loadmat('./HSI.mat')['data']
        labels = sio.loadmat('./HSI.mat')['gt']
        lidar = sio.loadmat('./LiDAR_DEM.mat')['LiDAR_DEM']
...
def get_adj(hsi,all, lidar, device, edge_num=10):
    ...
    # x = torch.cat([hsi, all, lidar], dim=-1)
    # corr_matrix = F.pairwise_distance(x.unsqueeze(1), x)
