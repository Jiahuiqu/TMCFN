...
def get_data(hsi, lidar, gt, graph_num, patchsize_HSI, patchsize_LiDAR, unlabeled_samples_num=None, num_samples_per_class=None, ratio=None ):
    '''
    获取gt中选出有标签的点的坐标，并均分成graph_num批
...
def train(labels):
    ...
    opti = torch.optim.Adam([{'params': model.vision_net.parameters(), 'lr': 0.0005},
                             {'params': model.projector_vision.parameters(), 'lr': 0.0005},
                             {'params': model.text_net.parameters(), 'lr': 0.00005}])
    ...
    for epoch in range(epoches):
        ...
        for i, (label, patch_hsi_all, patch_lidar_all, train_mask_,test_mask_, _, unlabeled_Patch_HSI, unlabeled_Patch_LiDAR) in enumerate(train_loader):
            ...
            if logprobs is not None and epoch+1 > warm_up_epoch:
                corr_pred = (torch.unsqueeze(train_pred, dim=0) == torch.unsqueeze(train_lab, dim=0)).float().detach()  # torch.Size([1, 1708])
                wron_pred = (1 - corr_pred)  ## 1 - 0 = 1,  1 - 1 = 0 torch.Size([1, 1708])

                if avg_accuracy[i] is None:
                    avg_accuracy[i] = torch.ones_like(corr_pred) * 0.5

                point_w = (avg_accuracy[i] - corr_pred)  # *(1*corr_pred + self.k*(1-corr_pred))
                if(len(logprobs.shape) == 4):
                    for m in range(logprobs.shape[-1]):
                        graph_loss += point_w * logprobs[:,:,:,m].exp().mean([-1,-2])  ## logprobs = (-(x1-x2).pow(2).sum(-1) * torch.exp(torch.clamp(self.temperature,-5,5))).reshape(x.shape[0],-1,self.k)  ##
            ...
            loss.backward()
            opti.step()
            model.vision_net.cc.update_moving_average()

            corr_pred = (train_pred == train_lab).float().detach().sum()  # torch.Size([1, 1708])
        ...
        if((epoch+1) % 5 ==0):
            ...
            if (OA_best < OA):
                ...
                sio.savemat(f'./results/Trento/pred_map_trento{OA_best}.mat', {'data': pred_map.numpy()})
...
if __name__ == '__main__':
    ...
    print(f"{patchsize_HSI} x {patchsize_LiDAR}")
    ...
    gt_labeled, labeled_data_hsi, labeled_data_lidar, All_labeled_Patch_HSI, All_labeled_Patch_LiDAR, train_mask, test_mask, pos_array, batch_data_id, unlabeled_Patch_HSI, unlabeled_Patch_LiDAR= \
                    get_data(hsi=hsi, lidar=lidar, gt=labels, graph_num=graph_num, patchsize_HSI=patchsize_HSI, patchsize_LiDAR=patchsize_LiDAR, num_samples_per_class=num_samples_per_class, unlabeled_samples_num=graph_num * 200, ratio=None)

    unlabeled_HSI_list = []
    unlabeled_LiDAR_list = []
    for i in range(graph_num):
        unlabeled_HSI_list.append(unlabeled_Patch_HSI[200 * i:200*(i+1)])
        unlabeled_LiDAR_list.append(unlabeled_Patch_LiDAR[200 * i:200*(i+1)])

    dataset = MyDataset(gt_labeled, All_labeled_Patch_HSI, All_labeled_Patch_LiDAR, train_mask, test_mask, batch_data_id, unlabeled_HSI_list, unlabeled_LiDAR_list)
    ...
    fw.write(f"{patchsize_HSI} x {patchsize_LiDAR}")
    print(f"{patchsize_HSI} x {patchsize_LiDAR}")
